---
title: "EDA - Relatório Rede Vocare"
author: "Arthur Cheib"
...
O escopo dos cruzamentos requisitados encontra-se formalizado em [arquivo word](https://docs.google.com/document/d/1tSrEHDH85UIyrhZ_kqPJVLL9fMH8ZMLoespqXv82aNY)

## 2.1 - Webnários Temáticos

Para elaboração deste Gráfico foram filtrados apenas os membros que estão estudando atualmente e que responderam ter disposição de colaborar em projetos igual a "Sim" ou "Talvez". Além disso, dividiu-se estes membros em dois grupos: I) Alta Identificação com a Rede; II) Baixa Identificação com a Rede. Essa divisão foi feita a partir da nota dada por estes membros no quesito identificação com a Rede. Se abaixo da nota média geral (média da nota dada neste memsmo quesitos pelos 63 respondentes do questionário), então foram classificados como "Baixa Identificação", se acima, então "Alta Identificação". 
...
escala_identificacao_webnario_02 <- mean(mean(censo_02$IDENTIFICACAO_REDE))
...
## 2.3 - Origem X Raça X Renda

```{r}

```

## 2.4 - Raça X Renda X Ocupação Atual
...
```{r}
censo_01 %>% 
  count(UNIVERSIDADE) %>% arrange(-n)
```


## 3.5 - Carreira Universitária

```{r}
censo_01 %>%
  count(ESTUDOS) %>% arrange(-n)
```

## 3.6 - Botineiros pelo Mundo
...
## 5.2 - Novos estudos/possibilidades